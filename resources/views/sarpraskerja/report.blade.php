<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Report Sarana Prasarana Kerja') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <form method="GET" class="flex gap-4 bg-white p-4 rounded shadow">
                <select name="kategori" class="border rounded px-2">
                    ...
                    @foreach(['Mesin Kerja', 'Komputer Kerja', 'Meja Kantor', 'Kursi Kantor', 'Mobil Operasional', 'Sepeda Motor Kerja', 'Alat Berat', 'Internet Kerja', 'Printer/Scanner', 'Telepon Kantor'] as $kat)
            ...
            {{-- Tabel detail --}}
            <div class="bg-white p-6 rounded shadow">
                <h3 class="font-bold text-lg mb-4">Detail Sarana Prasarana Kerja</h3>
                <table class="w-full border">
                    <thead class="bg-gray-100">
                        <tr>
                            <th class="border p-2">Nama Sarana</th>
                    ...
                    <tbody>
                        @forelse($data as $row)
                            <tr>
                                <td class="border p-2">{{ $row->nama_sarana }}</td>
                                <td class="border p-2">{{ $row->kategori }}</td>
                                <td class="border p-2">Rp {{ number_format($row->nilai_sarana,0,',','.') }}</td>
                                <td class="border p-2">{{ $row->tahun_perolehan }}</td>
                                <td class="border p-2">{{ $row->status_sarana }}</td>
