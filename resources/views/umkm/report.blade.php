        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <form method="GET" class="flex gap-4 bg-white p-4 rounded shadow">
                <select name="kategori" class="border rounded px-2">
                    ...
                    @foreach(['Pajak','usaha','induk','Lainnya'] as $kat)
                ...
                <select name="status_aset" class="border rounded px-2">
                    ...
                    @foreach(['Aktif','Rusak','Hilangan'] as $status)
                        <option value="{{ $status }}" @selected(request('status_umkm')==$status)>{{ $status }}</option>
                    @endforeach
                </select>
            ...
            <div class="bg-white p-6 rounded shadow">
                ...
                <p>Total umkm: <b>{{ $total }}</b></p>
                <p>Total jumlah: <b>Rp {{ number_format($totaljumlah,0,',','.') }}</b></p>
