    ...
    <div class="py-6" x-data="{
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-xl font-semibold text-gray-800">Data Bayi</h2>
                <form action="{{ route('bayi.index') }}" method="GET" class="flex gap-2 w-full md:w-1/2">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="🔍 Cari data bayi..." class="flex-1 px-4 py-2 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    ...
                    <table class="min-w-full text-sm border-collapse">
                        <thead class="bg-gray-100 text-gray-700 relative">
                            <tr>
                                ...
                                <th class="px-3 py-2 border">Ibu</th>
                            ...
                            <tbody>
                                @forelse ($bayi as $item)
                                    <tr class="hover:bg-gray-50 align-top">
                                        ...
                                        <td class="px-3 py-2 border align-top">{!! $item->nama_ibu !!}</td>
                                        <td class="px-3 py-2 border align-top">{!! $item->nama_bayi !!}</td>
                                        <td class="px-3 py-2 border align-top">{!! $item->tgl_lahir !!}</td>
                                        <td class="px-3 py-2 border align-top">{!! $item->jenis_kelamin !!}</td>
                                        <td class="px-3 py-2 border align-top">{!! $item->berat_badan !!}</td>
                                        <td class="px-3 py-2 border align-top">{!! $item->tinggi_badan !!}</td>
                ...
                {{-- Pagination --}}
                <div class="p-4">
                    {{ $bayi->appends(request()->all())->links() }}
                </div>
            </div>
        </div>
    </div>
