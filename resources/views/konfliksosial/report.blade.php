        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <form method="GET" class="flex gap-4 bg-white p-4 rounded shadow">
                <select name="kategori" class="border rounded px-2">
                    ...
                    @foreach(['Tanah', 'Pemukiman', 'Ekonomi', 'Sosial Budaya', 'Politik', 'Agraria', 'Lingkungan', 'Kriminalitas', 'Etnis', 'Agama', 'Pelayanan Publik'] as $kat)
                        <option value="{{ $kat }}" @selected(request('kategori')==$kat)>{{ $kat }}</option>
                    @endforeach
                </select>

                <select name="status_konflik" class="border rounded px-2">
                    <option value="">-- Status --</option>
                    @foreach(['Aktif', 'Terselesaikan', 'Berpotensi', 'Tidak Teridentifikasi'] as $status)
                        <option value="{{ $status }}" @selected(request('status_konflik')==$status)>{{ $status }}</option>
                    @endforeach
                </select>
            ...
            <div class="bg-white p-6 rounded shadow">
                ...
                <p>Total Kasus: <b>{{ $total }}</b></p>
                <p>Total Intensitas: <b>{{ number_format($totalIntensitas,0,',','.') }}</b></p>
            ...
            {{-- Tabel detail --}}
            <div class="bg-white p-6 rounded shadow">
                <h3 class="font-bold text-lg mb-4">Detail Konflik</h3>
                <table class="w-full border">
                    <thead class="bg-gray-100">
                        <tr>
                            <th class="border p-2">Nama Kelompok</th>
                            <th class="border p-2">Kategori</th>
                            <th class="border p-2">Intensitas</th>
                    ...
                    <tbody>
                        @forelse($data as $row)
                            <tr>
                                <td class="border p-2">{{ $row->nama_kelompok }}</td>
                                <td class="border p-2">{{ $row->kategori }}</td>
                                <td class="border p-2">{{ number_format($row->intensitas_konflik,0,',','.') }}</td>
                                <td class="border p-2">{{ $row->tahun_survey }}</td>
                                <td class="border p-2">{{ $row->status_konflik }}</td>
