        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('usaha_art.update', $item->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="mb-4">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="lapangan_usaha" id="lapangan_usaha" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                ...
                                @foreach([
                                    ...
                                    <option value="{{ $value }}" {{ old('lapangan_usaha', $item->lapangan_usaha) == $value ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="omset_usaha_bulan" id="omset_usaha_bulan" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                ...
                                @foreach([
                                    ...
                                    <option value="{{ $value }}" {{ old('omset_usaha_bulan', $item->omset_usaha_bulan) == $value ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('omset_usaha_bulan')
                        ...
                        <div class="mb-4">
                            ...
                            <select name="pendapatan_per_bulan" id="pendapatan_per_bulan" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                ...
                                @foreach([
                                    ...
                                    <option value="{{ $value }}" {{ old('pendapatan_per_bulan', $item->pendapatan_per_bulan) == $value ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="jumlah_pekerja" id="jumlah_pekerja" value="{{ old('jumlah_pekerja', $item->jumlah_pekerja) }}" min="1" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="status_kedudukan_kerja" id="status_kedudukan_kerja" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                ...
                                @foreach([
                                    ...
                                    <option value="{{ $value }}" {{ old('status_kedudukan_kerja', $item->status_kedudukan_kerja) == $value ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="surveyor_id" id="surveyor_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" {{ old('surveyor_id', $item->surveyor_id) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                                @endforeach
                            </select>
                        </div>

                        <button type="submit" class="mt-6 w-full bg-blue-600 text-white font-bold py-2 px-4 rounded hover:bg-blue-700">UPDATE</button>
                    </form>
                </div>
            </div>
        </div>
